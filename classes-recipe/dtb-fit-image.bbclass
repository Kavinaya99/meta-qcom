...
python do_generate_qcom_fitimage() {
    ...
    for key, compat_val in compat_flags.items():
        ...
        for ovl in overlays_list:
            ovl_name = os.path.basename(ovl)
            dtbo_name = ovl_name + ".dtbo"

            dtbo_path = os.path.join(dtb_dir, dtbo_name)
            if os.path.exists(dtbo_path):
                ovl_base = os.path.splitext(dtbo_name)[0]
                normalized_overlays.append(ovl_base)
                if dtbo_name not in ordered_files:
                    ordered_files.append(dtbo_name)

        if normalized_overlays:
            overlay_groups.setdefault(base, []).append(normalized_overlays)
            normalized_key = "+".join([base] + normalized_overlays)
            overlay_compats[normalized_key] = compat_val
